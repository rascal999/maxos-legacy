...
let
  cfg = config.maxos.tools.zsh;
  # In home-manager context, use standard home-manager options
  userConfig = {
    homeDirectory = config.home.homeDirectory;
    monorepoDirectory = "${config.home.homeDirectory}/mgp-monorepo";
    workspaceDirectory = "${config.home.homeDirectory}/workspace";
  };
  
  # Validate dependencies exist before referencing them
  dependenciesValid = true; # Zsh has no hard dependencies
  
in {
  options.maxos.tools.zsh = {
    ...
    assertions = [
      {
        ...
        message = "Zsh shell configuration has no hard dependencies";
      }
    ];
  };
}
