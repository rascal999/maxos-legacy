...
in
{
  ...
  gpsbabel = toolGenerator.generateSimpleTool {
    toolName = "gpsbabel";
    packages = pkgs: with pkgs; [ gpsbabel ];
    description = "GPSBabel GPS data format converter";
  };
  ...
  # DevOps tools
  faas-cli = toolGenerator.generateSimpleTool {
    toolName = "faas-cli";
    packages = pkgs: with pkgs; [ faas-cli ];
